...
- Xử lý giọng nói: STT (PhoWhisper) + wake-word (hey_jarvis).
- RAG dạng graph chỉ là nền tảng kỹ thuật hỗ trợ hội thoại.
...
## Ghi chú phát triển
- `need_research` được ưu tiên cho các truy vấn realtime trong planner.
- Kết quả cuối cùng có thể bị reset bởi `refresh_state`, nên khi debug nên stream state theo bước.
